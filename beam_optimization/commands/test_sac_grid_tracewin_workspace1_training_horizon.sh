#!/usr/bin/env bash
# Test all SAC grid policies on TraceWin workspace 1 using the same episode
# horizon used to train each policy. Save only the final render and phase space.
set -euo pipefail
...
TEST_ROOT="beam_optimization/results/test/sac_penalty_horizon_grid_tracewin_workspace1_training_horizon_final_render"
...
TRACEWIN_PROJECT="beam_optimization/env/tracewin_env/tracewin/TraceWin_workspace/CB_newMRMS_RFQ_Fields_1.ini"
TEST_SEED=2026
...
for POLICY in "${POLICIES[@]}"; do
  ...
  case "$LABEL" in
    ...
    new_h30_*) HORIZON=30 ;;
    *)
      echo "Cannot infer training horizon from configuration name: $LABEL"
      exit 1
      ;;
  esac
  ...
  echo "Testing $LABEL with its training horizon: $HORIZON steps"
